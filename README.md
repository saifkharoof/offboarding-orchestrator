...
A full annotated transcript — including a genuine forceful kill of a running
...
runnable [`scripts/demo.py`](scripts/demo.py) (stdlib only — no bash, no
curl; works the same on Windows, macOS, and Linux):

```bash
python scripts/demo.py                            # watch it happen
python scripts/demo.py > docs/demo-transcript.txt  # regenerate the transcript
```

Nothing in it is simulated: every PID printed is real (each CLI call runs as
its own child process and is waited on), and the "process is dead" check is
a real failed connection after a real, forceful kill — `SIGKILL` on
POSIX, `TerminateProcess` on Windows — not a comment
saying so.
